screenshots:
DAG task graph in Airflow UI 
![alt text](image-5.png)
fig1. dag_graph.png         
Example of a successful DAG run (all tasks green)
![alt text](image-4.png)
fig2. successful_run.png    
Example of a failed task (red task + error log)
![alt text](image-3.png)
fig3. failed_run.png        
Gmail notification received on task failure
<img width="837" height="458" alt="image" src="https://github.com/user-attachments/assets/dd6cdc78-5031-4f81-8f40-11d47eb6a0d1" />
fig4. failure_email.png     
- backfill.png          
Full architecture diagram (Postgres → Airbyte → BigQuery → dbt)
  <img width="543" height="294" alt="image" src="https://github.com/user-attachments/assets/e83d4521-40d8-4c5a-aae4-af054a63d148" />
fig5. architecture.png      

